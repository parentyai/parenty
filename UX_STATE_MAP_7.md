...
## City Pack Watch 表示ルール（非UX状態）

### 背景
- City Pack の watch state は UXの7状態ではなく、表示上の結果ラベルである。
- 表示を最小化し、責任境界と規約違反のリスクを抑える。

### 説明
- state は `ok / risk / unknown` の3値のみとし、UNKNOWN は正常状態として扱う。
- ユーザー表示は「判定結果」と「最終確認日」のみ。理由・根拠・探索過程は管理UIのみ参照可。
- 表現は SSOT 5-8 のコピー制約に従い、断定/保証に見える文言を避ける。
- watch state は UX状態の追加・置換に使用しない。

### 結論
- City Pack の表示は結果と確認日だけに固定し、UX状態は7状態のまま維持する。

### 補足
- 詳細仕様は SSOT 5-8 と `docs/ops/city_pack_auto_generation_spec.md` を参照する。
