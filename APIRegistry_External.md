...
| zocdoc | 医療 | `https://developers.zocdoc.com/` | `https://developers.zocdoc.com/` | `https://www.zocdoc.com/about/terms` | `https://www.zocdoc.com/about/privacy` | （非公開） | Docs/Terms/Privacy を週次監視（Terms/Privacyは手動レビュー想定） | 事実: zocdoc.com は機械巡回で 403 になりやすい（URLは正だが監視手段が課題）。参考: 旧Docs候補（`zocdoc.com/about/api`）は機械巡回で403 |
...
| noaa_nws | 天気/警報 | `https://www.weather.gov/documentation/services-web-api` | `https://api.weather.gov` | `https://www.weather.gov/disclaimer` | `https://www.noaa.gov/privacy-policy`（HOLD） | （無料） | Docs/Endpoint/Terms/Privacy を週次監視 | 事実: `noaa.gov/privacy-policy` は 404（[仮説] Privacy URL到達不能のためHOLD）。TODO: 代替URL確定まで利用前提にしない。 |
