...
### Marketing Copy Guideline（City Pack Watch）

#### 背景
- City Pack の watch 結果は責任境界が明確な表現で扱う必要がある。
- 断定や保証に見える表現は、法務/運用のリスクになる。

#### 説明
- 使用可/禁止表現の正は SSOT 5-8 に固定する。
- 派生側では表現の新規追加を行わず、参照導線のみを持つ。
- 表示は結果と最終確認日の範囲に限定する。

#### 結論
- Marketing Copy の拘束は SSOT 5-8 を正とし、本書は参照導線に限定する。

#### 補足
- 表現の追加が必要な場合は SSOT 改定へ戻す。
